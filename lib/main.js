...
exports.main = function (aOptions, aCallbacks) {
	...
	pageMod.PageMod({
		...
		contentScript    : self.data.load("AutoPatchWork.js"),
		onAttach         : function (aWorker) {
			aWorker.on("message", function(aMessage){
				var name = aMessage.name;
				var data = aMessage.data;
				if (aMessage.name) data.message = aMessage.name;
				handleMessage(data, {}, function(aData){
					aWorker.postMessage({name: name, data: aData});
				});
			});
		},
	});
